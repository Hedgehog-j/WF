...
//防抖函数避免resize频繁发生影响性能
function antiShake(func,wait,immediate){
	/*
		func:需要进行防抖的函数
		wait：需要等待的间隔
		immediate:布尔值，是否需要立即执行一次
	*/
	var timeoutId;
	return function(){
		var that=this,
		...
		var activeNow = immediate && !timeoutId;
		if(timeoutId){
...
//绑定resize事件以利于让瀑布流自适应
EventUtil.addHandler(window,"resize",antiShake(function(){WFDemo.waterfallActive("waterfall","WFBox")},300,"ture"));
